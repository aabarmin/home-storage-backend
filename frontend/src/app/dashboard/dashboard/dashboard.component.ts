...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    ...
    const flatChange$ = this.formGroup.get('flat')
      ?.valueChanges as Subject<String>;
    this.years$ = this.dataService.findRecordsForFlat(flatChange$).pipe(
    ...
    const yearChange$ = this.formGroup.get('year')
      ?.valueChanges as Observable<Number>;
    this.dashboard$ = this.dataService
      .findRecordsForFlatAndYear(flatChange$, yearChange$)
      .pipe(map((records) => this.toDashboard(records)));
